...
async function createSection() {
  ...
  try {
    const { data } = await client.post('/forum/sections', { title: secTitle.value.trim(), description: secDesc.value.trim() || null, color: secColor.value })
    sections.value.push(data)
    secTitle.value = ''; secDesc.value = ''; secColor.value = '#c8742a'; showNewSection.value = false
  ...
  <div class="mx-auto max-w-6xl">
    ...
    <template v-else-if="tab === 'sections'">
      <div v-if="auth.can('forum.post')" class="mb-4">
        <button v-if="!showNewSection" class="btn-outline" @click="showNewSection = true"><AppIcon name="forum" :size="16" /> Новый раздел</button>
        <div v-else class="card space-y-3 p-4">
          ...
          <div class="flex gap-2">
            <button class="btn-primary" :disabled="savingSection || !secTitle.trim()" @click="createSection">{{ savingSection ? '…' : 'Создать' }}</button>
            <button class="btn-ghost" @click="showNewSection = false">Отмена</button>
          </div>
        </div>
      </div>
      <div v-if="!sections.length" class="card p-10 text-center text-ink-700/50">Разделов пока нет</div>
      <div v-else class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        <div v-for="s in sections" :key="s.id" class="card p-4">
          <div class="flex items-start justify-between gap-2">
            ...
            <button class="shrink-0 text-ink-700/30 hover:text-red-600" title="Удалить" @click="removeSection(s)"><AppIcon name="trash" :size="15" /></button>
          </div>
    ...
    <template v-else>
      ...
      <div v-else class="card overflow-hidden">
        ...
        <div class="hidden items-center gap-4 border-b border-parchment-200 px-4 py-2.5 text-xs uppercase tracking-wide text-ink-700/50 sm:flex">
          ...
          <div class="w-16 shrink-0 text-right">Активность</div>
        </div>
        <template v-for="(t, i) in list" :key="t.id">
          ...
          <RouterLink :to="{ name: 'forum-topic', params: { id: t.id } }"
            ...
            <div class="w-12 shrink-0 text-right text-xs text-ink-700/50 sm:w-16">{{ fmtAgo(t.last_activity) }}</div>
          </RouterLink>
        </template>
      </div>
    </template>
